...
#pragma omp parallel for
            for (int i = 0; i < img_out.width * img_out.height / 3; ++i)
            {
                float var_r = (float)img_in.img_r[i] / 255;
...
#pragma omp section
        {
#pragma omp parallel for
            for (int i = img_out.width * img_out.height / 3; i < 2 * img_out.width * img_out.height / 3; ++i)
            {
                float var_r = (float)img_in.img_r[i] / 255;
                float var_g = (float)img_in.img_g[i] / 255;
                float var_b = (float)img_in.img_b[i] / 255;
...
#pragma omp section
        {
#pragma omp parallel for
            for (int i = 2 * img_out.width * img_out.height / 3; i < img_out.width * img_out.height; ++i)
            {
                float var_r = (float)img_in.img_r[i] / 255;
                float var_g = (float)img_in.img_g[i] / 255;
                float var_b = (float)img_in.img_b[i] / 255;
...
#pragma omp section
        {
#pragma omp parallel for
            for (int i = 0; i < result.w * result.h / 3; ++i)
            {
                float H = img_in.h[i];
                float S = img_in.s[i];
...
#pragma omp section
        {
#pragma omp parallel for
            for (int i = result.w * result.h / 3; i < 2 * result.w * result.h / 3; ++i)
            {
                float H = img_in.h[i];
                float S = img_in.s[i];
                float L = img_in.l[i] / 255.0f;
...
#pragma omp section
        {
#pragma omp parallel for
            for (int i = 2 * result.w * result.h / 3; i < result.w * result.h; ++i)
            {
                float H = img_in.h[i];
                float S = img_in.s[i];
                float L = img_in.l[i] / 255.0f;
...
#pragma omp section
        {
#pragma omp parallel for
            for (int i = 0; i < img_out.w * img_out.h / 3; ++i)
            {
...
#pragma omp section
        {
#pragma omp parallel for
            for (int i = img_out.w * img_out.h / 3; i < 2 * img_out.w * img_out.h / 3; ++i)
            {
                unsigned char r = img_in.img_r[i];
                unsigned char g = img_in.img_g[i];
                unsigned char b = img_in.img_b[i];
...
#pragma omp section
        {
#pragma omp parallel for
            for (int i = 2 * img_out.w * img_out.h / 3; i < img_out.w * img_out.h; ++i)
            {
                unsigned char r = img_in.img_r[i];
                unsigned char g = img_in.img_g[i];
                unsigned char b = img_in.img_b[i];
...
#pragma omp parallel for
            for (int i = 0; i < img_out.w * img_out.h / 3; ++i)
            {
                int y = (int)img_in.img_y[i];
                int cb = (int)img_in.img_u[i] - 128;
                int cr = (int)img_in.img_v[i] - 128;
